...
description: How to use an advanced calculator as a "Game Boy" for CHIP-8!
image: /assets/img/hp48.png
categories:
  - Blog
tags:
  - CHIP-8
...
However, both CHIP-48 (`c48`) and SUPER-CHIP (`SCHIP`/`GCHIP`) are subtly incompatible with many CHIP-8 games written for other platforms. They're not completely backwards compatible and can't run all games from the 70s and 80s, for example. You can read more about these incompatibilities in [Chromatophore's excellent and detailed write-up](https://github.com/Chromatophore/HP48-Superchip/blob/master/README.md). She also patched `SCHIP` and `GCHIP` to support these games, and these patched versions are called `SCHPC` (SUPER-CHIP Compatibility) and `GCHPC`, respectively. Of course, many actual SUPER-CHIP games won't run on these... So it's best to get both flavors.
...
> Press <kbd>I/O</kbd> (<kbd>↰</kbd><kbd>PRG</kbd>) and then press <kbd>SETUP</kbd> (<kbd>F</kbd>).
>
> The setup should look like this:
>
> ![](/assets/img/48s_io.png)
>
> When you're done, press <kbd>I/O</kbd> (<kbd>↰</kbd><kbd>PRG</kbd>) again, and it'll go back to the main screen with the I/O menu open at the bottom..
{: .prompt-warning }

> Press <kbd>I/O</kbd> (<kbd>↱</kbd><kbd>1</kbd>) and choose "Transfer...".
>
> The setup should look like this:
>
> ![](/assets/img/48g_io.png)
{: .prompt-tip }
...
For Windows, I ended up just getting the good old HyperTerminal. That program used to come with Windows all the way until Windows XP. It's not included anymore, but you can [find it online](https://www.hilgraeve.com/hyperterminal-trial/) and run it fine on Windows 11 (it's just an EXE and a DLL). Set up with the corresponding settings as on the calculator. You can probably also use [PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html).
...
<figure class="half">
    <img src="/assets/img/cosmac-vip-keypad.png">
    <img src="/assets/img/hp48_keypad.png">
...
[Octojam](http://octojam.com) was an annual game jam, where enthusiasts got together to make CHIP-8 games. Sadly it’s no longer active, but we’re a small ex-community who can probably still give advice!