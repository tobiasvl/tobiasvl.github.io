...
{: .prompt-warning}\r\rIn the CHIP-8 interpreter for the original COSMAC VIP, this instruction did the following: It put the value of `VY` into `VX`, and then shifted the value in `VX` 1 bit to the right (`8XY6`) or left (`8XYE`). `VY` was not affeced, but the flag register `VF` would be set to the bit that was shifted out.
...
> Ambiguous instruction!
{: .prompt-warning}\r\rIn the original COSMAC VIP interpreter, this instruction jumped to the address `NNN` plus the value in the register `V0`. This was mainly used for "jump tables", to quickly be able to jump to different subroutines based on some input.
...
{: .prompt-warning}\r\r### `DXYN`: Display
...
{: .prompt-warning}\r\rOne area where people get confused is whether sprites should wrap if they go over the edge of the screen. The answer is yes and no.
...
{: .prompt-warning}\r\r### `FX0A`: Get key
...
> Ambiguous instruction!
{: .prompt-warning}\r\rThese two instructions store registers to memory, or load them from memory, respectively.
